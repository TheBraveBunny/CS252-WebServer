...
void dump_form(FILE * fssock) {
    ...
    fprintf(fssock, "<FORM ACTION=\"jj-mod.so\">%c",LF);
	...
	fprintf(stderr, "httprun 1b\n");
	//shutdown( fssock, 2 );
	fclose( fssock );
	fprintf(stderr, "httprun 1c\n");
}
...
httprun(int ssock, char * query_string)
{
  ...
  if ( fssock == NULL ) {
	...
	fprintf(stderr, "httprun 5\n");

    fprintf(fssock,"My name is %s, and I would like to have the following%c",
            name,LF);
    fprintf(fssock,"order delivered to %s:%c%c",address,LF,LF);
    for(x=0;x<12;x++)
        if(subs & (1 << x))
            fprintf(fssock,"\t(1) %s%c",sublist[x],LF);
    for(x=0;x<6;x++)
        if(slims & (1 << x))
            fprintf(fssock,"\t(1) %s Slim Jim%c",slimlist[x],LF);
    for(x=0;x<2;x++)
        if(sides & (1 << x))
            fprintf(fssock,"\t(1) %s%c",sidelist[x],LF);
    for(x=0;x<4;x++)
        if(drinks & (1 << x))
            fprintf(fssock,"\t(1) %s%c",poplist[x],LF);
    fprintf(fssock,"%cPlease feel free to call me at %s if there is any%c",LF,
            phone,LF);
    fprintf(fssock,"problem. Thank you.%c%c.%c",LF,LF,LF);

	fprintf(stderr, "httprun 6\n");
